...
def pagamentos_por_aluno(id_aluno):
    ...
    try:
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute('SELECT id_pagamento, id_aluno, data_pagamento, valor_pago, forma_pagamento, referencia, status FROM "Pagamento" WHERE id_aluno = %s', (id_aluno,))
        pagamentos = cur.fetchall()
        cur.close()
        conn.close()
        return jsonify([
...
def pagamentos_por_status(status):
    ...
    try:
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute('SELECT id_pagamento, id_aluno, data_pagamento, valor_pago, forma_pagamento, referencia, status FROM "Pagamento" WHERE status = %s', (status,))
        pagamentos = cur.fetchall()
        cur.close()
        conn.close()
        return jsonify([
